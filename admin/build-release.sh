...
if [ "X${1}" = "X-p" ]; then
	...
	Passing -p means we copy all product files to the SOEST ftp directory
	Passing -m means we only copy the macOS bundle to the SOEST ftp directory
	[Default only produces the products but do not place them]
	EOF
	exit 1
fi
if [ ! -d admin ]; then
